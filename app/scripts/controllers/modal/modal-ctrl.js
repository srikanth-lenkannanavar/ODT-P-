...
angular
		...
		.controller(
				'ModalCtrl',
				function($scope, modalservice, CommonService, $routeParams,
						restservice, envconfig, $location, $filter, $cookies,
						$interval, $rootScope,  $route,
						$timeout, $window,$sce,geocodeservice,YourQuoteService)
				{
            ...
            if (!icons[iconStr]) {
                if(iconStr === "C"){
					...
					$scope.getLocation = function()
					{
						...
						var start = $scope.bookingObj.retailerpostalcode;
						var end = $scope.bookingObj.postcode;

						var loc = new google.maps.LatLng($scope.jobgps.lat,
								$scope.jobgps.lng);
						...
						var waypts = [
						{
							location : new google.maps.LatLng(
									$scope.jobgps.lat, $scope.jobgps.lng),
							stopover : true
						} ];
						var PtsArray = [];
								...
								status)
						{
							...
							{	
			                    ...
			                    for (var i=0; i < legs.length;i++){
			                        ...
			                        if(i == 0){
 										var collectFromDet = $scope.bookingObj.retailername+"<br>"+$scope.bookingObj.retaileraddressline1+","+$scope.bookingObj.retailercity+" | "+$scope.bookingObj.retailerpostalcode;
                            			toolTipDetails = "Collect from:<br>"+collectFromDet;
			                        }else{
			                    ...
			                    var destDet = "Delivery to:<br>"+$scope.bookingObj.deliveryrefaddress1+","+$scope.bookingObj.deliveryrefaddresstown+" | "+$scope.bookingObj.postcode;			                    
					...
					$scope.cancelBooking = function()
					{
						...
						var cancelBookingUrl = envconfig.get('host')
